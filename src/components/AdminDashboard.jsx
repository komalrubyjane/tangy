...
function Sparkline({ data, color = "#D4AF37", width = 120, height = 40 }) {
    ...
    <div style={{
      ...
      <motion.div
        ...
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          <div style={{ fontSize: "0.65rem", letterSpacing: "0.3em", color: "#D4AF37", textTransform: "uppercase", fontFamily: "monospace", marginBottom: 8 }}>Tangy Sessions</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <motion.button onClick={handleLogin} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
            style={{
              padding: "14px", background: "#D4AF37", color: "#fff", border: "none", borderRadius: 8,
  ...
  const FILTER_BTN = (active, label, val, setter) => (
    <button onClick={() => setter(val)}
      style={{
        ...
        background: active === val ? "#D4AF37" : "rgba(255,255,255,0.04)",
        color: active === val ? "#fff" : "rgba(255,255,255,0.5)",
        border: `1px solid ${active === val ? "#D4AF37" : "rgba(255,255,255,0.08)"}`,
      ...
      <div style={{ display: "flex", minHeight: "100vh" }}>
        <aside style={{
          ...
          <div style={{ padding: "0 24px 28px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
            <div style={{ fontSize: "0.6rem", letterSpacing: "0.3em", color: "#D4AF37", textTransform: "uppercase", fontFamily: "monospace", marginBottom: 4 }}>Dashboard</div>
            <img
              src="/logo.svg"
            ...
            {TABS.map(t => (
              <button key={t.id} onClick={() => { setTab(t.id); setSearch(""); }}
                style={{
                  ...
                  border: `1px solid ${tab === t.id ? "rgba(212, 175, 55,0.3)" : "transparent"}`,
                ...
                {t.id === "artists" && pendingArtists > 0 && (
                  <span style={{ marginLeft: "auto", background: "#D4AF37", color: "#fff", borderRadius: 10, padding: "1px 7px", fontSize: "0.65rem", fontWeight: 700 }}>{pendingArtists}</span>
                )}
              </button>
            ))}
          </nav>
            ...
            {tab === "overview" && (
              <motion.div key="overview" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 16, marginBottom: 32 }}>
                  <StatCard label="Total Revenue" value={`₹${(totalRevenue / 1000).toFixed(1)}K`} sub={`From ${bookings.filter(b => b.status === "confirmed").length} bookings`} color="#D4AF37" icon="💰" sparkData={[42, 58, 71, 65, 83, 91, 105, 98, 120, 115, 132, 142]} trend="+24%" />
                  ...
                  <StatCard label="Volunteers" value={volunteers.length} sub={`${pendingVols} pending review`} color="#FFD700" icon="🤝" sparkData={[2, 3, 3, 4, 4, 5, 6, 6, 7, 8, 8, 9]} trend="+3" />
                ...
                <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, overflow: "hidden" }}>
                  <div style={{ padding: "18px 24px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <button onClick={() => setTab("bookings")} style={{ background: "none", border: "none", color: "#D4AF37", cursor: "pointer", fontSize: "0.78rem" }}>View all →</button>
                  </div>
                      ...
                      <tbody>
                        {bookings.slice(0, 5).map((b, i) => (
                          <motion.tr key={b.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                            ...
                            <td style={{ padding: "14px 20px", fontFamily: "monospace", fontSize: "0.75rem", color: "#D4AF37" }}>{b.id}</td>
                      ...
                      <div style={{ height: 6, background: "rgba(255,255,255,0.06)", borderRadius: 3, overflow: "hidden" }}>
                        <motion.div
                          ...
                          style={{
                            ...
                            background: ev.sold / ev.capacity > 0.8 ? "#ef4444" : ev.sold / ev.capacity > 0.5 ? "#f59e0b" : "#D4AF37"
                          }} />
                      </div>
                    </div>
                  ))}
                        ...
                        <AnimatePresence>
                          {filteredBookings.map((b, i) => (
                            <motion.tr key={b.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ delay: i * 0.03 }}
                              ...
                              onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,0.02)"}
                              onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                              <td style={{ padding: "14px 18px", fontFamily: "monospace", fontSize: "0.73rem", color: "#D4AF37", whiteSpace: "nowrap" }}>{b.id}</td>
                  ...
                  <AnimatePresence>
                    {filteredVolunteers.map((v, i) => (
                      <motion.div key={v.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ delay: i * 0.05 }}
                        ...
                        <div style={{ width: 44, height: 44, borderRadius: "50%", background: "rgba(212, 175, 55,0.15)", border: "1px solid rgba(212, 175, 55,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Bebas Neue', sans-serif", fontSize: "1.1rem", color: "#D4AF37", flexShrink: 0 }}>
                    ...
                    <motion.div key={ev.name} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 16, marginBottom: 20 }}>
                        ...
                        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                          ...
                          <button style={{ padding: "7px 16px", background: "rgba(212, 175, 55,0.1)", border: "1px solid rgba(212, 175, 55,0.25)", borderRadius: 6, color: "#D4AF37", cursor: "pointer", fontSize: "0.78rem", fontFamily: "inherit", transition: "all 0.2s" }}
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 16, marginBottom: 20 }}>
                        {[
                          ["Revenue", `₹${ev.revenue.toLocaleString("en-IN")}`, "#D4AF37"],
                          ["Tickets Sold", ev.sold, "#06b6d4"],
                          ["Capacity", ev.capacity, "#FFD700"],
                        ...
                        <div style={{ height: 8, background: "rgba(255,255,255,0.06)", borderRadius: 4, overflow: "hidden" }}>
                          <motion.div
                            ...
                            style={{
                              ...
                              background: `linear-gradient(to right, #D4AF37, ${ev.sold / ev.capacity > 0.8 ? "#ef4444" : "#06b6d4"})`,
                            }} />
                        </div>
                      </div>
                    </motion.div>
                  ...
                  <AnimatePresence mode="popLayout">
                    {filteredArtists.map((a, i) => (
                      <motion.div key={a.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ delay: i * 0.05 }}
                        ...
                        {/* Avatar */}
                        <div style={{ width: 48, height: 48, borderRadius: "50%", overflow: "hidden", border: `2px solid ${a.color || "#D4AF37"}`, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(255,255,255,0.03)" }}>
                          {a.avatar ? (
                          ...
                          <span style={{ display: a.avatar ? "none" : "flex", fontFamily: "'Bebas Neue', sans-serif", fontSize: "1.2rem", color: a.color || "#D4AF37" }}>
          ...
          >
            <motion.div
              ...
              style={{
                ...
                boxShadow: "0 30px 70px rgba(0,0,0,0.8), 0 0 50px rgba(212, 175, 55, 0.15), inset 0 1px 1px rgba(255,255,255,0.05)",
                borderRadius: 24,
                width: "100%",
                maxWidth: 480,
                padding: "36px 40px",
